...
- When changing the core transform logic, add a corresponding `testN` function (see the existing `test1`...`test42`) and register it in the `tests` array inside `runMocoSelfTests()` — this is the only regression safety net in the project.
...
5. Find all `Customer` entries (independent of invoices/dossier entries — customers live in their own `Task`, e.g. `Id=Kunden`); for each whose `AddressData > Name` exactly matches a key in `CONFIG.intercompanyByCustomerName`, ensure an `Intercompany` field with the mapped value exists (same `ensureField` mechanics as step 2, inserted after `DefaultCurrency`). No name match is not an error — most customers aren't intercompany customers.
